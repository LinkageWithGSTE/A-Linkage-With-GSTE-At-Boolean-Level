...
head = BitVec('head', MSBA + 1)
tail = BitVec('tail', MSBA + 1)
...
for i in range(LAST + 2):
    if i == 0:
        ...
        is_empty = Not(Bool('empty'))
    if i == LAST + 1:
        is_full = Bool('full')
    else:
        is_full = Not(Bool('full'))
    f = And(
        tail == head+i ,
        is_empty,
        is_full
...
for i in range(LAST+1):
    ...
    print(f)
    keys = [head, tail, empty, full, mem[head+i]]
    vertexs.append(
        (2 * i + 4, f, keys)
    )
